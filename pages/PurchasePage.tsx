                ...
                <p className="text-center text-slate-300 mb-8">Estás a un paso de apartar tus boletos para: {raffle.title}</p>
                 <div className="bg-background-secondary p-8 rounded-lg border border-slate-700/50 shadow-lg">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                        ...
                        <div>
                            <label htmlFor="phone" className="block text-sm font-medium text-white mb-1">Teléfono (10 dígitos)</label>
                            <input id="phone" type="tel" {...register('phone', { required: 'El teléfono es requerido', pattern: {value: /^\d{10}$/, message: 'Ingresa un teléfono válido de 10 dígitos' } })} className="w-full bg-slate-800 border border-slate-700 rounded-md py-2 px-3 text-white focus:ring-accent focus:border-accent" />
                            {errors.phone && <p className="text-red-400 text-xs mt-1">{errors.phone.message}</p>}
                        </div>
                        <div>
                            <label htmlFor="state" className="block text-sm font-medium text-white mb-1">Estado</label>
                            <input id="state" {...register('state', { required: 'El estado es requerido' })} className="w-full bg-slate-800 border border-slate-700 rounded-md py-2 px-3 text-white focus:ring-accent focus:border-accent" />
                            {errors.state && <p className="text-red-400 text-xs mt-1">{errors.state.message}</p>}
                        </div>
                         <div className="mt-8 pt-6 border-t border-slate-700 text-center">
                            <p className="text-2xl font-bold text-white">Total a Pagar: ${total.toFixed(2)} MXN</p>
